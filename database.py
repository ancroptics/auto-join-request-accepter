...
def fix_database_url(url):
    """Convert direct Supabase URL to pooler URL if needed."""
    if not url:
        return url
    parsed = urlparse(url)
...
async def get_user_rank(user_id):
    ...
    async with p.acquire() as conn:
        rank = await conn.fetchval(
            "SELECT COUNT(*)+1 FROM users WHERE referral_count > (SELECT COALESCE(referral_count,0) FROM users WHERE user_id=$1)", user_id)
        return rank or "N/A"
...
async def get_recent_join_requests(limit=10):
    ...
    async with p.acquire() as conn:
        rows = await conn.fetch(
            "SELECT jr.*, u.first_name, u.username FROM join_requests jr LEFT JOIN users u ON jr.user_id=u.user_id ORDER BY jr.created_at DESC LIMIT $1", limit)
        return [dict(r) for r in rows]
