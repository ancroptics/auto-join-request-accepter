    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute("""
            DO $$ BEGIN
                ...
                END IF;
                IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='channels' AND column_name='added_by') THEN
                    ALTER TABLE channels ADD COLUMN added_by BIGINT;
                END IF;
            END $$;
        """)
...
async def get_new_user_ids(days=1):
    pool = await get_pool()
    async with pool.acquire() as conn:
        rows = await conn.fetch(f"SELECT user_id FROM users WHERE created_at > NOW() - INTERVAL '{days} days'")
        return [r["user_id"] for r in rows]

async def get_active_user_ids(days=1):
    pool = await get_pool()
    async with pool.acquire() as conn:
        rows = await conn.fetch(f"SELECT DISTINCT user_id FROM join_requests WHERE created_at > NOW() - INTERVAL '{days} days'")
        return [r["user_id"] for r in rows]
...
async def get_leaderboard(limit=10):
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT user_id, username, first_name, referral_count, balance FROM users ORDER BY referral_count DESC LIMIT $1",
            limit
        )
        return [dict(r) for r in rows]

async def add_channel(channel_id, title=None, username=None, added_by=None):
    pool = await get_pool()
    async with pool.acquire() as conn:
        try:
            await conn.execute("ALTER TABLE channels ADD COLUMN IF NOT EXISTS added_by BIGINT")
        except:
            pass
        await conn.execute(
            "INSERT INTO channels (channel_id, title, username, added_by) VALUES ($1, $2, $3, $4) ON CONFLICT (channel_id) DO UPDATE SET title = $2, username = $3",
...
async def get_channels_by_user(user_id):
    pool = await get_pool()
    async with pool.acquire() as conn:
        rows = await conn.fetch("SELECT * FROM channels WHERE added_by = $1 ORDER BY added_at DESC", user_id)
        return [dict(r) for r in rows]
...
async def get_join_request_stats():
    pool = await get_pool()
    async with pool.acquire() as conn:
        rows = await conn.fetch("SELECT status, COUNT(*) as c FROM join_requests GROUP BY status")
